...
def invoke_swarm(task: str, context: str = "", max_iterations: int = None) -> Optional[str]:
    """Invoke the Zen-Swarm loop for atomic code changes."""
    try:
        from swarm_ctrl import run_zen_loop, MAX_ITERATIONS
        if max_iterations:
            CONFIG.get("zen_swarm", {})["max_iterations"] = max_iterations
    except ImportError:
        return "ERROR: swarm_ctrl.py not found in dev/"
    
    print(f"\n{'='*50}")
    print(f"Zen-Swarm invoked: {task[:50]}...")
    print(f"{'='*50}")
    
    return "Zen-Swarm loop initiated - requires file_path/search/replace args"
